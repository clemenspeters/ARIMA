'''
Training data source: https://www1.nyc.gov/site/tlc/about/tlc-trip-record-data.page
Detect anomalies in: https://github.com/numenta/NAB/blob/master/data/realKnownCause/nyc_taxi.csv
'''
...
result_dir = 'results/taxi_nyc_numenta_{}-{}'.format(encoding_method, '_'.join(str(x) for x in order))
window_size = 100
...
def visualize_labelled_series(anomaly_windows, show=True):
    ...
    title = 'Numenta NYC taxi dataset'
    plt.title(title)
...
def get_test_data():
    test_file = 'data/Numenta/data/realKnownCause/nyc_taxi.csv'
    return  pd.read_csv(
                test_file,
                header=0,
                index_col=0,
...
def generate_data_and_features(out_folder):
    ...
    fn = '{}/plot_{}_taxi_numenta_file.png'.format(out_folder, file_count)
    data_test = get_test_data()
    visualize(data_test, file_count=1, file_name=fn, show=False)
...
def show_raw_data():
    file_names = get_sorted_file_names()
    selected_files = file_names[18:48]
...
# labelled_features_fn = 'results/taxi_nyc_numenta_ARMA-2_2/features-test_ARMA_labelled_0_25.csv'
